...
public class CadastraProduto2 extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CadastraProduto2 frame = new CadastraProduto2();
	...
	private void cadastrarProduto(Produto prod) throws ParseException {
		...
		if(prod instanceof Alimenticio) {
			Alimenticio alim = (Alimenticio)prod;
			alim.setDataValidade(tfValidade.getText());
			alim.setDateProducao(tfProducao.getText());
		} else { 
			Eletrodomestico elet = (Eletrodomestico)prod;
			elet.setCodigoFornecedor(tfCodForn.getText());
			elet.setNomeFornecedor(tfNomeForn.getText());
		}
	}
}
