...
public class Ex03 extends JPanel {
		...
		btOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				try {
					...
					JOptionPane.showMessageDialog(getParent(), "O 2º número não pode ser ZERO");
				} catch(NumberFormatException ex) {
					JOptionPane.showMessageDialog(getParent(), "Número(s) inválido(s)");
				} finally {
					JOptionPane.showMessageDialog(getParent(), "Operação concluida");
				}
			}
		});
		botoes.add(btOk);
	...
	public static void main(String[] args) {
		...
		tela.getContentPane().add(new Ex03(), "Center");
		tela.pack();
